...
class Request {
    ...
    public $pagesStr = "admin,signp,login,reset,profile,account,settings,,search,terms,privacy,contact,upload,";
    ...
    public function url($url = '', $param = array(), $direct = false) {
        ...
        if($url) {
            $mainUrl = ($this->config('permalink') or $direct) ? $url : '?p='.$url;
    ...
    public  function load($route, $active = true, $filter = false, $permission = false) {
        ...
        if (!class_exists($controllerClass)) exit('Failed : Controller does not exists');
